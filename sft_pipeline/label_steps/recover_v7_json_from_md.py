...
from bo_to_doc_v7 import split_into_steps  # type: ignore  # noqa: E402
    ...
    for seq_path in iter_sequence_files(data_dir):
        ...
        for idx, (start, end) in enumerate(step_ranges, start=1):
            ...
            row = {
                ...
                "step_text_v6": step_text,  # Compatibility for existing SFT builders.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Recover v7 step JSONL from existing MD without LLM calls.")
    ...
    manifest = recover_v7_json_from_md(
        Path(args.data_dir),
        Path(args.md_dir),
        Path(args.output),
