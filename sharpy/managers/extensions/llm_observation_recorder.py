...
class LLMObservationRecorder(ManagerBase):
    ...
    def _build_snapshot(self) -> Dict:
        ...
        return self._build_snapshot_for(self.ai)

    def capture_observation_bundle(self, bot=None) -> Dict:
        """Capture structured snapshot and English text for the given bot state."""
        ai = bot or self.ai
        if ai is None:
            return {"structured": {}, "text": ""}
        snapshot = self._build_snapshot_for(ai)
        return {
            "structured": snapshot,
            "text": self._generate_english_text_obs(snapshot),
        }

    def _build_snapshot_for(self, ai) -> Dict:
        saved_ai = self.ai
        self.ai = ai
        try:
            return {
                "time": round(ai.time, 2),
                "time_formatted": ai.time_formatted,
