...
| 执行 | `ActLLMOngoingTasks`（Sharpy Act 自判每帧续产） | `ExecutionScheduler`（独立 waiter 槽 + 三档优先级 + 资源保留同档超车 / 科技等待 / RUNNING 超时 / SCV 预移动） |
...
- **推进规则**：初始传 Step 1；当 `actions` 列表全 terminal 或仅剩 deferred 时传下一个 step（`append`）；waiter 不影响推进；最后一个 step 执行完后重复传最后一个 step。
...
- **要点**：把口语/别名规范成数据库标准名（`resolve_alias` + `is_known_terran_entity` 校验），
  无法识别的实体直接丢弃；supply depot 作为普通宏观目标保留，是否托管给算法由 Stage5 决定。
