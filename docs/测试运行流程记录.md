...
`BO_list/<race>/<name>/BO.json` 整条 action 序列一次性灌入 `ExecutionScheduler`；
`Executor Agent`（train/addon/morph 选执行单位）仍会被调用。
...
- `.log` 中应出现 `>>> BO LIST: '<name>' loaded (<N> actions)`，紧随其后是 `[BO list] installed <N> actions into scheduler.`，且全程**不应**出现 `>>> MACRO PIPELINE START` 行。
- `.json` 轨迹中应出现 `trigger_reason="bo_list_loaded"` 与 `trigger_reason="bo_list_installed"` 两条事件；不应再有 `naming_raw` / `ordering_raw` 字段。
