...
> **基线复现**：`game_records/marine_rush_full/20260617_132254_mr_hard_air/`（修复前，终局 2 兵营）  
> **回归复现**：`game_records/marine_rush_full/20260617_141733_mr_retest/`（局部修复后，终局累计 3 兵营但第 3 座延迟 88s）  
> **关联代码**：`SC2_Agent/execution/scheduler.py`、`sharpy/plans/acts/grid_building.py`、`dummies/generic/universal_llm_bot.py`  
> 本文做**问题定位、机制说明、局部修复评估与完整修复方案**。
...
**当前代码**（2026-06-17 局部修复后，已去掉 `- not_ready`，但问题仍在）：

```152:160:sharpy/plans/acts/grid_building.py
        if (
...
**原始代码**（基线复现 `132254_mr_hard_air` 时）：

```python
count + (self.pending_build(self.unit_type) - self.cache.own(self.unit_type).not_ready.amount) >= self.to_count
```
...
> **回归备注（§15）**：2026-06-17 仅实施 §11 中 P1/P2 的部分等价物及 P4/P5；§10.1、§11.1-2、DONE 校验均未做。`141733_mr_retest` 证实终局数量可「碰巧」对齐，但单条 `x3` 仍只下 2 单——**不能以终局 total 作为修复验收标准**。
...
| **§15** | 局部修复后 x3 仍只下 2 单，第 3 座靠 Step3 append 补救 | `141733_mr_retest` | **高（未根治）** |
...
预期（完全修复前）：Installed 次数 > GridBuilding 行数，终局 total < LLM 累计 quantity；完全修复后二者应一致，且第 N 座 GridBuilding 不应依赖后续 macro 周期补救。

---

## 15. 局部修复尝试与回归（2026-06-17）

在基线复现（§2，`132254_mr_hard_air`）之后，对执行层做了**部分**改动并重新跑局（`141733_mr_retest`）。本节记录改了什么、测出了什么、**为何用户感知「问题仍在」**。

### 15.1 已实施的局部改动

| 代号 | 文件 | 改动摘要 | 对应 §11 项 |
|------|------|----------|-------------|
| P1 | `scheduler.py` → `set_actions(append)` | 同 `action_name` 且未 DONE 的 `CAT_BUILD`/`CAT_ADDON`：**合并 quantity**，并 `existing._act.to_count += pa.quantity` | §11.3 |
| P2 | `universal_llm_bot.py` → `_guard_prefetch_build_quantity` | `mode=append` 时，若队列中已有同类建造 `active_names` / `active_build_types`，**从 pairs 中剔除**新条目 | §11.5 |
| P3 | `grid_building.py:152-154` | early-return 公式由 `count + pending - not_ready` 改为 `count + pending` | §11.6（方向存疑，见下） |
| P4 | `scheduler.py` → `_existing_plus_en_route` | SCV 工单目标点 1.0 内已有结构则**不计入** en_route，减轻防重复闸 double-count | §11.4 |
| P5 | `scheduler.py` → `set_actions(replace)` | 丢弃旧 action 前调用 `clear_worker()` | §11.7 / §10.3 |

相关代码位置：

```99:119:SC2_Agent/execution/scheduler.py
        if mode == "append":
            kept = [a for a in self.actions if not a.is_terminal()]
            merged_new = []
...
```868:899:dummies/generic/universal_llm_bot.py
    def _guard_prefetch_build_quantity(self, pairs):
        ...
...
| 指标 | 基线 `132254_mr_hard_air` | 回归 `141733_mr_retest` |
|------|---------------------------|-------------------------|
| 结果 | Victory | Defeat（15:52） |
| `[GridBuilding] BARRACKS` 行数 | **2** | **3** |
| 我方终局 BARRACKS 累计 | total **2** | total **3**（lifetime，终局已被打掉） |
| Step1 `BARRACKS x3` 内建满 3 座 | ❌ 只建 2 | ❌ 只建 2（01:03、01:15） |
| 第 3 座来源 | 未建成 | Step3 prefetch **append** `BARRACKS x1` → 02:43 GridBuilding |
| 第 2→第 3 座间隔 | — | **~88 秒**（01:15 → 02:43） |
| Step2 prefetch 重复兵营 | append `BARRACKS x2` | **被 P2 拦住**（Installed 无 BARRACKS） |
| Step3 prefetch 重复兵营 | append `BARRACKS x1` | **仍 append** `BARRACKS x1` |

**结论**：局部修复让「终局兵营数」从 2 变为 3，但**未消除根因**——第一条 `TERRANBUILD_BARRACKS x3` 仍在 2 座后被提前 DONE，第 3 座靠后续 macro 周期「补救」，且严重滞后。

### 15.3 回归局关键时间线（`141733_mr_retest`）

| 游戏时间 | 事件 | 说明 |
|----------|------|------|
| 00:00 | `initial_step` → `BARRACKS x3` replace | 规划正确 |
| 00:57 | `single_waiter_prefetch` cycle=2 | `BARRACKS x3 (0/3 issued) [WAITING]` |
| 00:57 | Step2 Installed append | **无 BARRACKS**（P2 guard 生效）；LLM 曾识别 `BARRACKS x2` 但被过滤 |
| 01:03 | GridBuilding #1 | 第 1 座 |
| 01:15 | GridBuilding #2 | 第 2 座；**`BARRACKS x3` act 未继续第 3 次下单** |
| 02:03 | prefetch cycle=3 | `pending: TERRANBUILD_BARRACKS [WAITING]`（原 x3 已非完整形态） |
| 02:03 | Installed append | `TERRANBUILD_BARRACKS x1`（P2 **未拦住**） |
| 02:43 | GridBuilding #3 | 第 3 座，比第 2 座晚 **88s** |
| 03:33 | 观测 | `Completed: ... 3 BARRACKS` |
| 15:52 | 终局 | Defeat；`Own BARRACKS total: 3`（累计建造数） |

### 15.4 局部修复为何不够

**P2（prefetch guard）只拦了一半**

- Step2：队列里仍有 `TERRANBUILD_BARRACKS` 在 `active_names` → 新 `BARRACKS x2` 被剔除 ✓  
- Step3：Step1 的 `BARRACKS x3` 已在 2 座后被标 **DONE**，guard 认为「无活跃同类建造」→ `BARRACKS x1` 作为**新 PlannedAction** 入队 ✗  

guard 按 **队列 action_name** 判断，未按 **场上实际数量 vs LLM 本步目标** 判断，无法覆盖「假完成后的补救」场景。

**P1（append merge）无法挽救已 DONE 的 x3**

- merge 仅对 `not existing.is_terminal()` 生效；`BARRACKS x3` 在 2 座后 DONE → 后续 quantity 无法合并回同一条 act  
- Step3 的 `x1` 成为独立条目，触发延迟的第三次 GridBuilding

**P3（grid_building 公式改动）未击中要害**

- 去掉 `- not_ready` 可能使 `count + pending` **更大**，更易触发 early-return（与 §10.1 `get_count` 底层 bug 叠加时行为更难预测）  
- 回归局仍出现「x3 只下 2 单」，说明 **P0 级修复（§10.1、§11.1-2）尚未实施**

**P4（en_route 去重）**

- 减轻防重复闸误触发，但无法修复「GridBuilding 提前 return True」或「x3 在 act 内不继续下单」

### 15.5 「问题仍在」的 Agent 层定义

即使终局累计建了 3 兵营，以下现象仍属**同一 Bug 未根治**：

1. **单条 PlannedAction 执行不完整**：`Installed BARRACKS x3`，同一 act 仅 2 次 `[GridBuilding]`  
2. **依赖后续 macro 补救**：第 3 座非 Step1 自然完成，而是 Step3 append 的独立 `x1`  
3. **建造节奏失真**：第 3 兵营比第 2 兵营晚 ~1.5 分钟，连续生产建筑类 macro 严重受损  
4. **Prefetch 与执行状态不同步**：LLM 已进入 Step3，执行层处于「x3 假完成 + 孤立 WAITING / 新 x1」中间态  

回归检查时不应只看终局 `GameAnalyzerEnd BARRACKS total`，还应比对：

```powershell
# 第一条 BARRACKS xN 安装后，GridBuilding 次数是否等于 N（在下一步 prefetch 之前）
Select-String -Path <match_dir>\*.log -Pattern 'Installed.*BARRACKS x3|\[GridBuilding\] BARRACKS'

# 第 3 座是否出现在第一条 x3 之后、下一次 prefetch 之前（理想）还是更晚
```
...
| 优先级 | 项 | 回归验证 |
|--------|-----|----------|
| **必须** | §10.1 `get_count` 修复 | 未做；计数链仍不可靠 |
| **必须** | GridBuilding DONE 条件改为 **`_existing_plus_en_route >= to_count`**（或 `get_count >= to_count`），废弃 `count + pending` 估算 | 未做；x3 仍只下 2 单 |
| **必须** | Scheduler DONE 时校验 **`场上数量 >= _act_target_count`**，不满足则保持 RUNNING | 未做 |
| 已部分有效 | P2 prefetch guard | Step2 ✓，Step3 ✗；需改为按场上数量补差 |
| 已部分有效 | P1 append merge | 无法合并已 DONE 的 act |
| 待观察 | P3 公式改动 | 建议回滚或随 §10.1 一并重写 |
| 建议保留 | P4 en_route 去重、P5 replace clear_worker | 有益但非主因 |

### 15.7 回归日志摘录（`141733_mr_retest`）

**P2 拦住 Step2 重复兵营：**

```text
Stage3 mapped: {'TERRANBUILD_BARRACKS': 2, ...}
Installed (append): ['COMMANDCENTERTRAIN_SCV x1', 'UPGRADETOORBITAL_ORBITALCOMMAND x1',
                     'TERRANBUILD_SUPPLYDEPOT x1', 'BARRACKSTRAIN_MARINE x6']
# 注意：无 TERRANBUILD_BARRACKS
```

**Step3 仍 append，第 3 座延迟建造：**

```text
Installed (append): [..., 'TERRANBUILD_BARRACKS x1', ...]
01:03  [GridBuilding] BARRACKS at (119.5, 33.5)
01:15  [GridBuilding] BARRACKS at (112.5, 24.5)
02:43  [GridBuilding] BARRACKS at (112.5, 32.5)
```

**终局：**

```text
[GameAnalyzerEnd] Own units:
[GameAnalyzerEnd] BARRACKS  total: 3  alive: 0  dead: 3
Result: Defeat
```
