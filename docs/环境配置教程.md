...
建议用 `run_custom.py` 指定短 `--record-dir` 与 `--match-id` 做冒烟：

```powershell
$env:SC2PATH='C:\Program Files (x86)\StarCraft II'
$env:SC2_GAME_TIME_LIMIT='60'
$env:PYTHONUTF8='1'
New-Item -ItemType Directory -Force -Path .\gr\smoke | Out-Null
python run_custom.py `
  ...
  --record-dir .\gr\smoke `
  --match-id smoke `
  --instruct '打一波 以 大和为主的攻击' `
  --top-model DeepSeek-V4-flash `
  --mid-model DeepSeek-V4-flash `
  --down-model DeepSeek-V4-flash `
  --increment-model DeepSeek-V4-flash `
...
| Windows 日志路径过长 | 用 `run_custom.py --record-dir .\gr\smoke --match-id smoke` 指定短路径 |
...
python run_custom.py -m KairosJunctionLE -p1 universal_llm.terran -p2 ai.terran.easy.macro `
  --record-dir .\gr\smoke --match-id smoke --force-strategy battle_cruisers
```
