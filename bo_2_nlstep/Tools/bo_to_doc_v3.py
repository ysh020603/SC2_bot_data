"""
BO Action Sequence -> Natural Language Build Order Document (Slang v3)

Based on v1, with two changes:
- No cross-step context (BuildContext removed) -- each step is self-contained
- SC2 community slang / abbreviations for all units, buildings, upgrades

Usage:
    python bo_to_doc_v3.py                    # process all 10 BOs (5 concurrent)
    python bo_to_doc_v3.py --bot banshees     # single BO
    python bo_to_doc_v3.py --max-workers 3    # 3 concurrent
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BO Action Sequence -> Natural Language Docs (Slang v3)")
    ...
    parser.add_argument("--output-dir", type=str,
                        default=None,
                        help="Output directory for bo_docs (default: Tools/bo_docs_slang)")
    args = parser.parse_args()

    data_dir = args.data_dir
    output_dir = args.output_dir or os.path.join(_TOOLS_DIR, "bo_docs_slang")
