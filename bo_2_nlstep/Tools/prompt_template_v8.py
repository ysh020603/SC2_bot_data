...
into natural language build order step descriptions (Macro-Control Fuzzy v8).

Based on v7, with changes:
- Step descriptions keep v7 No Ordinals, slang style, and no cross-step context.
- Each normal step receives macro-control cues inferred from the action list.
- The model blends those cues into natural coach-style language, helping
  downstream reasoning models pace worker production, depot planning, and
  gas-economy capacity without using concrete observation values.
- Summary and Final Step rules are intentionally inherited from v7 unchanged.
"""

# ---------------------------------------------------------------------------
# System prompt -- v8 adds macro-control fuzzy hints to v7
# ---------------------------------------------------------------------------
...
12. **Blend in macro-control fuzzy hints**: The prompt may include "Soft Macro-Control Cues" inferred from the actions in this step. Use them as subtle coaching language for exactly these macro-control ideas:
   - Worker saturation: pace SCV production from worker current/ideal and the current economy task.
   - Supply buffer: pace depot construction from supply headroom plus the current step's SCV/Marine/production demand.
   - Gas capacity: describe Refineries as strengthening gas income or gas flexibility for upcoming tech, without forcing a one-to-one binding to a specific unit or upgrade.

   These hints must stay natural and fuzzy, not programmatic. Do NOT output the cues as a list. Do NOT write formulas, thresholds, rigid if/else logic, or actual observation values, because no current observation is provided during labeling.

   GOOD: "Keep SCV production rolling while the economy still has room to saturate."
   GOOD: "Add depot room for the next SCV and Marine cycle without overcommitting minerals to unused cap."
   GOOD: "Add a Refinery to strengthen the gas economy for the upcoming tech path."
   BAD:  "If workers_current < workers_ideal, train SCVs."
   BAD:  "Since supply is 28/150, build two depots."
   BAD:  "This Refinery is specifically for the next Banshee."
   BAD:  "Worker saturation cue: check current/ideal workers."

13. **Do not add excluded analysis in normal steps**: Do NOT mention enemy, scout/scouting, pressure, harassment/harass, threat, rush, combat advantage, army advantage, hold/poke/move-out branching, active queues, idle production, queue conflicts, or generic tech-readiness rules. Keep the step focused on macro execution, production, worker saturation, supply headroom, and gas capacity.
"""
