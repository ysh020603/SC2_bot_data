"""
BO Action Sequence -> Natural Language Build Order Document (Balanced Phase Summary v5)

Based on v4, with changes:
  - Summary restructured to 3-sentence balanced format (Early / Mid / Late)
  - Strategic extrapolation removed from Summary
  - Strategic extrapolation becomes a standalone Final Step [Strategic Follow-up] 
    appended to the end of Details
  - Step descriptions unchanged from v4/v3 (Slang style, no cross-step context)
...
from prompt_template_v5 import (
    SYSTEM_PROMPT,
...
# ---------------------------------------------------------------------------
# Single trajectory processor (v5: Balanced Summary + Final Step)
# ---------------------------------------------------------------------------
def process_trajectory(
    ...
    generate 3-sentence balanced summary, generate final follow-up step, assemble output."""
    ...
    # -----------------------------------------------------------------------
    # Generate 3-sentence Balanced Summary (v5: early/mid/late, NO extrapolation)
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Generate Strategic Follow-up Final Step (v5: standalone, was sentence 4 in v4)
    # -----------------------------------------------------------------------
    print(f"  [{bot_folder}] Generating Strategic Follow-up Final Step...")
    ...
    for s_out in step_outputs:
        md_content += f"{s_out}\n\n"
    # Append the Strategic Follow-up as the final entry in Details
...
def main():
    ...
    parser = argparse.ArgumentParser(description="BO Action Sequence -> Natural Language Docs (v5 Balanced Phase Summary)")
    ...
    parser.add_argument("--output-dir", type=str,
                        default=None,
                        help="Output directory for bo_docs (default: Tools/bo_docs_balanced)")
    args = parser.parse_args()

    data_dir = args.data_dir
    output_dir = args.output_dir or os.path.join(_TOOLS_DIR, "bo_docs_balanced")
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in as_completed(future_to_bot):
            ...
            try:
                ...
                success_count += 1
                print(f"  [{bot_folder}] COMPLETED ({result['total_steps']} steps + follow-up)")
