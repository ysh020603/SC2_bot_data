#!/usr/bin/env bash
# OLD + NEW 各 7 个 Terran 策略 × 3 张地图 × 3 种族 × 5 难度，对手 Macro AI。
# 采集规则与 bo_collection_runs/2026-06-24_terran_6bots_3maps_macro 一致。
# 用法: bash tools/run_terran_old_new_7bots_3maps_macro_collect.sh
...
SESSION="${TMUX_SESSION:-sc2_terran_old_new_7bots_macro_20260705}"
DATE_TAG="${DATE_TAG:-2026-07-05}"
OUTPUT_ROOT_OLD="${OUTPUT_ROOT_OLD:-${ROOT}/bo_collection_runs/${DATE_TAG}_terran_old7bots_3maps_macro}"
OUTPUT_ROOT_NEW="${OUTPUT_ROOT_NEW:-${ROOT}/bo_collection_runs/${DATE_TAG}_terran_new7bots_3maps_macro}"
WORKERS="${WORKERS:-40}"
...
for BATCH in OLD NEW; do \
  if [ \"\$BATCH\" = OLD ]; then \
    ...
    BOTS=\"banshees battle_cruisers bio cyclones marine_rush rusty two_base_tanks\"; \
  else \
    OUTPUT_ROOT=\"${OUTPUT_ROOT_NEW}\"; \
    BOTS=\"raven_screams yamato_rust_fleet rusty_bio_mines blueflame_locks stim_rush_relay old_rusty_anvil two_base_matrix_tanks\"; \
  fi; \
...
echo "  每批 315 局 (7 bots × 3 races × 5 diffs × 3 maps)，共 630 局"
